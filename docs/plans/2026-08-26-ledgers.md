# 원장 구현 계획 — buyers · suppliers · intakeReviews

> **For agentic workers:**  Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 바이어·제조사 원장과 기존 원천의 판정 오버레이를 서버 전용으로 만들고, 수기로 등록·수정·판정할 수 있는 화면을 붙인다.

**Architecture:** 읽기는 서버 컴포넌트가 Admin SDK로 직접 하고, 쓰기는 `withAdmin` route handler를 지난다. 제조사 마스터에는 담당자·역량·공급 방식만 두고 거래별 역할·소유권·견적은 계획 5의 딜에 둔다. `orders` 등 기존 원천은 수정하지 않고 `intakeReviews`가 테스트 여부와 `raw|qualified|rejected` 판정을 덧씌운다. 입력 검증은 Zod 스키마 하나가 서버와 타입 양쪽을 담당한다.
...
- `buyers`·`suppliers`·`intakeReviews` 규칙은 **모든 클라이언트에게 `if false`** (계획 1 Task 3에서 적용). 새 컬렉션 규칙과 테스트는 같은 커밋에 들어간다
...
| `src/lib/schemas/intake-review.ts` (신규)                                | 원천 판정 Zod 스키마 + 타입                                      |
| `src/lib/repo/suppliers.ts` (신규)                                       | Firestore 접근. 서버 전용                                        |
| `src/lib/repo/buyers.ts` (신규)                                          | Firestore 접근. 서버 전용                                        |
| `src/lib/repo/intake-reviews.ts` (신규)                                  | 기존 원천을 건드리지 않는 판정 오버레이                          |
...
| `src/app/api/admin/intake-reviews/[source]/[externalId]/route.ts` (신규) | 판정 저장                                                        |
...
| `src/app/admin/(dash)/intakes/page.tsx` (신규)                           | raw·qualified·rejected 검토 목록                               |
...
- Produces:
  ...
  - `listSuppliers(): Promise<Supplier[]>`, `findSupplierByEmail(email): Promise<Supplier | null>`, `createSupplier(input, actor): Promise<string>`, `updateSupplier(id, input, actor): Promise<void>`
...
Run: `npm test`
Expected: `schemas.test.ts` 전건 PASS
...
const EMPTY_CONTACT = { name: "", title: "", email: "", phone: "", channel: "email" as const };
const EMPTY = {
  companyName: "",
...
| 담당자 둘과 복수 역량 등록                     | 두 담당자와`contactEmails`·역량이 저장된다        |
| 수정 → 인증·역량 변경                        | 목록에 반영된다                                      |
| 소유권·원가 필드를 강제로 전송                | 저장되지 않는다                                      |
...
### Task 6: 기존 원천 판정 오버레이

**Files:**

- Create: `src/lib/schemas/intake-review.ts`
- Create: `src/lib/repo/intake-reviews.ts`
- Create: `src/app/api/admin/intake-reviews/[source]/[externalId]/route.ts`
- Create: `src/app/admin/(dash)/intakes/page.tsx`
- Modify: `firestore.rules`
- Modify: `tests/rules/backoffice.test.mjs`
- Modify: `tests/schemas.test.ts`

**Interfaces:**

- Produces:
  - `intakeReviewInputSchema`, `type IntakeReview`
  - `getIntakeReview(source, externalId)`, `setIntakeReview(source, externalId, input, actor)`
  - `intakeReviewId(source, externalId): string`

- [ ] **Step 1: 판정 스키마 테스트를 먼저 쓴다**

검증할 것:

- `source`는 `order|sampleRequest|contact|koreaLead|message`만 통과
- `status='qualified'|'rejected'`이면 공백이 아닌 `reason`이 필수
- `isTest=true`이면 공백이 아닌 `isTestReason`이 필수
- `raw`는 아직 판정 전이므로 reason 없이 통과
- 문서 ID의 `/`·`:`·공백은 원문 그대로 조합하지 않고 base64url로 인코딩

- [ ] **Step 2: 스키마와 결정적 ID를 구현한다**

`src/lib/schemas/intake-review.ts`:
...
- [ ] **Step 3: 저장소와 route handler를 구현한다**

`setIntakeReview()`는 URL의 `source`·`externalId`를 body 값보다 우선하며, Zod 검증 뒤 아래 모양으로 `set(..., { merge: true })` 한다.
...
`status='qualified'`라고 딜을 자동 생성하지 않는다. 계획 5의 딜 생성 화면이 이 판정을 읽고 명시적 사용자 동작으로 `dealId`를 연결한다.

- [ ] **Step 4: 규칙과 에뮬레이터 테스트를 같은 커밋에 넣는다**

```text
match /intakeReviews/{id} { allow read, write: if false; }
```

비로그인·일반 로그인·문서 소유자 세 경우의 read/write가 모두 거부되는지 확인한다.

- [ ] **Step 5: 인테이크 검토 화면을 만든다**

`/admin/intakes`는 기존 `orders`·`sampleRequests`·`contact`·`koreaLeads`와 `intakeReviews`를 서버에서 합친다. 기본 필터는 `raw`다. 각 행에 원천, 생성 시각, 바이어 식별값, 현재 판정, 테스트 여부를 보이고 다음 액션을 둔다.

```text
[정상 리드로 승인] [거절] [내부 테스트]
```

세 액션 모두 사유 입력을 요구한다. `orders` 원문은 수정하지 않는다. 메일 기반 인테이크는 계획 4가 `source='message'`로 같은 화면에 연결한다.

- [ ] **Step 6: 집계 경계 테스트를 추가한다**

순수 함수 `isQualifiedIntake(review)`가 다음을 만족해야 한다.
...
필드가 없다는 이유로 과거 문서를 고객 또는 테스트로 추측하지 않는다. 판정이 없으면 `raw`다.
...
```bash
git add firestore.rules tests src/lib/schemas/intake-review.ts src/lib/repo/intake-reviews.ts src/app/api/admin/intake-reviews "src/app/admin/(dash)/intakes"
git commit -m "feat(admin): 원천 제출 판정 오버레이"
git push -u origin feat/ledgers
gh pr create --base dev --title "feat(admin): 바이어·제조사·인테이크 원장" --body "스펙 docs/backoffice-spec.md 4.1 / 작업 순서 4"
```
...
- **거래 조건과 원가** — `supplierEngagements`의 역할·MOQ·납기와 `private/finance`의 견적·원가는 계획 5. 제조사 마스터에는 넣지 않는다
- **자동 연결** — 수집기가 발신자로 바이어·제조사를 찾는 건 계획 4. `findBuyerByEmail()`·`findSupplierByEmail()`을 여기서 미리 만든다
