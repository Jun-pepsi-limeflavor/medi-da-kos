...
void main() {
  ...
  float rim = 1.0 - smoothstep(uShineSize - uShineFade, uShineSize + uShineFade + 1e-4, phi); float line = gaussianLine(d, uThickness);
  float edgeClamp = 1.0 - smoothstep(0.5 * uPx, 3.0 * uPx, abs(d)); float hi = line * rim * edgeClamp * uIntensity;
...
export function SpecularButton({
  ...
  textColor = "#f5f5f5", lineColor = "#ffffff", baseColor = "#525252", bg, bgHover, intensity = 1,
  shineSize = 10, shineFade = 40, thickness = 1, speed = 0.35, followMouse = true,
  proximity = 250, autoAnimate = false, className, disabled = false, ...buttonProps
}: SpecularButtonProps) {
  ...
  useEffect(() => {
    ...
    const movePointer = (event: PointerEvent) => {
      ...
      const target = Math.max(0, 1 - distance / Math.max(propsRef.current.proximity, 1));
      proximityTarget = target * target * (3 - 2 * target);
    };
    ...
    const render = (now: number) => {
      ...
      const shouldFollow = current.followMouse && pointerAngle !== null && (!current.autoAnimate || proximityTarget > 0);
      ...
      const targetBrightness = current.autoAnimate ? 1 : proximityTarget;
  ...
  const style = {
    ...
    ...(bg ? { "--sb-bg": bg } : {}),
    ...(bgHover ? { "--sb-bg-hover": bgHover } : {}),
  } as React.CSSProperties;

  return (
    <button ref={buttonRef} type="button" disabled={disabled} className={`${styles.button} ${styles[size]}${className ? ` ${className}` : ""}`} style={style} {...buttonProps}>
      <span ref={effectRef} className={styles.effect} aria-hidden="true" />
