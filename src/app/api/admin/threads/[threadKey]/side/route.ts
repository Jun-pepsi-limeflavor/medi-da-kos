...
export const PATCH = withAdmin(async (req, actor) => {
  ...
  try {
    await correctThreadSide(decodeURIComponent(threadKey), body as { side: unknown; reason: string }, actor);
    ...
    if (err instanceof Error && err.message.includes("정정 사유")) {
      return NextResponse.json(
        { error: err.message },
        { status: 400 }
      );
